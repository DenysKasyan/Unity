...
public class SkinCat : MonoBehaviour
{
    ...
    private void ApplySkinBonus(int index)
    {
        ...
        if (Array.Exists(epicSkins, b => b == SkinBt[index]))
        {
            ...
            GameManager.bestEnergyRegenInterval = Mathf.Min(GameManager.bestEnergyRegenInterval, 2.4f);
        }
        else if (Array.Exists(legendarySkins, b => b == SkinBt[index]))
        {
            // Якщо купив легендарний — порівнюємо з поточним best
            GameManager.bestEnergyRegenInterval = Mathf.Min(GameManager.bestEnergyRegenInterval, 0.5f);
        }

        GameManager.energyRegenInterval = GameManager.bestEnergyRegenInterval;
    }
